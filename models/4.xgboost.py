...
def train_xgboost(X_train, y_train, param_grid=None):
    model = xgb.XGBRegressor(objective='reg:squarederror', eval_metric='rmse', use_label_encoder=False)
    if param_grid:
...
best_model, best_params = train_xgboost(X_train, y_train, param_grid)
print(f'Best Parameters: {best_params}')
y_pred = evaluate_model(best_model, X_test, y_test)

# Predictions for plotting
plot_results(y_test, y_pred, title='XGBoost: True Values vs Predictions')
plot_residuals(y_test, y_pred, title='XGBoost: Residuals Plot')
plot_histogram_of_residuals(y_test, y_pred, title='XGBoost: Histogram of Residuals')

# Plot learning curves for XGBoost model
print("\nPlotting Learning Curves for XGBoost Model:")
plot_learning_curves(X_train, y_train, X_test, y_test, xgb.XGBRegressor, best_params)
